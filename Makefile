...
dev-status: dev-db-ps
	...
	@echo ""
	@echo "--- Redis (Docker) ---"
	@docker-compose ps redis-sentinel 2>/dev/null || echo "  Redis Sentinel not running"
	@docker-compose ps redis-master 2>/dev/null || echo "  Redis Master not running"
	@docker-compose ps redis-slave 2>/dev/null || echo "  Redis Slave not running"
	@echo ""
